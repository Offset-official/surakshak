...
def intrusion_detector(frame, camera_name):
    ...
    if all([
        ...
        logger.debug("Camera boundary not fully defined. Any detected person triggers lockdown.")
        x1_ratio = None
        x2_ratio = None
        y1_ratio = None
        y2_ratio = None
    ...
    if lockdown_needed:
        with lockdown_lock:
            ...
            if not system_config.SystemConfig.lockdown:
                logger.info("Lockdown needed. System is not in lockdown. Proceeding to initiate lockdown.")
