...
def state_switch(InferenceSchedule):
    while True:
        ...
        if getattr(inactive_schedule, now_weekday_name):
            ...
            if SystemConfig.lockdown:
            ...
            elif (now_hour, now_min) == (
                inactive_schedule_start_hour,
                inactive_schedule_start_min,
            ):
                SystemConfig.set_intrusion("INACTIVE")
                logger.info(
                    "System state switched to INACTIVE. Logger sleeping for 120 seconds."
                )
                ...
                inactive_schedule_end_hour,
                inactive_schedule_end_min,
            ):
                SystemConfig.set_intrusion("ACTIVE")
                logger.info(
                    "System state switched to ACTIVE. Logger sleeping for 120 seconds."
                )
